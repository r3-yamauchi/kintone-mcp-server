...
## 概要
kintone REST APIでレコードを検索・取得する際に使用するクエリー記法について説明します。
...
注記:
- \`is empty\` / \`is not empty\` が使用できるのは、主に「文字列（複数行）」および「添付ファイル（FILE）」です。
- 「文字列（1行）」では \`is empty\` / \`is not empty\` は使用できません（空欄判定は未対応）。
...
- 添付ファイル（FILE）フィールドでも \`is empty\` / \`is not empty\` により「ファイルの有無」で絞り込み可能です（クエリ式でのレコード検索に限る）。ただし、プロセス管理やアプリアクションの条件、レコード/フィールドのアクセス権の条件では使用できません
- 文字列検索で使用できない記号があります（詳細はkintoneヘルプを参照）
...
### パフォーマンスの最適化
1. インデックスが設定されているフィールドで検索する
2. \`like\`演算子より\`=\`や\`in\`演算子を優先する
3. 必要なレコード数だけ取得するよう\`limit\`を適切に設定する

### クエリの可読性
1. 複雑な条件は括弧でグループ化する
2. 論理演算子（\`and\`/\`or\`）の優先順位に注意する
3. フィールドコードは分かりやすい名前を使用する
...
- \`offset\`の最大値は10,000
- 10,000件を超えるレコードを取得する場合は、複数回のAPIコールが必要
- ソートで使用できるフィールドには制限がある
- \`like\`演算子で使用できない記号がある
- 添付ファイルに対する \`is empty\` / \`is not empty\` は「レコード検索のクエリ式」でのみ使用可能。プロセス管理・アプリアクションの条件、レコード/フィールドのアクセス権の条件では使用不可
- 「文字列（1行）」では \`is empty\` / \`is not empty\` は使用不可
